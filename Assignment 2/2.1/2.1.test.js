...
// prettier-ignore
const { Event_, DataType, WeatherData, WeatherPrediction, Temperature, Wind, Precipitation, CloudCoverage, TemperaturePrediction, PrecipitationPrediction, WindPrediction, CloudCoveragePrediction, DateInterval, WeatherForecast, WeatherHistory } = require("./2.1.js");
...
test(TEMPERATURE + " convertion 2", () => {
  ...
  t = t.convertToF();
  expect(t.getUnit()).toBe(FAHRENHEIT);
...
test("PrecipitationPrediction test", () => {
  ...
  let pp1 = pp.convertToMM();
  expect(pp1).toEqual(pp);
...
test("WindPrediction test", () => {
  let wp = WindPrediction(Date(2020, 3, 3, 0, 0, 0), LONDON, WIND, MS, 10, 13, [
    EAST,
    SOUTH,
  ]);
  let wp1 = wp.convertToMPH();
  expect(wp1.getUnit()).not.toEqual(wp.getUnit());
  expect(wp1.getUnit()).toEqual(MPH);
  expect(wp1.getMax()).toBeCloseTo(29.0802);
  expect(wp1.getMin()).toBeCloseTo(22.3694);

  wp1 = wp.convertToMS();
  expect(wp1.getUnit()).toEqual(wp.getUnit());
  expect(wp1.getMax()).toBeCloseTo(wp.getMax());
  expect(wp1.getMin()).toBeCloseTo(wp.getMin());

  let data = Wind(Date(2020, 3, 3, 0, 0, 0), LONDON, WIND, MS, 11.231, SOUTH);
  expect(wp.matches(data)).toEqual(true);

  data = Wind(Date(2020, 3, 3, 0, 0, 0), LONDON, WIND, MS, 11.231, NORTH);
